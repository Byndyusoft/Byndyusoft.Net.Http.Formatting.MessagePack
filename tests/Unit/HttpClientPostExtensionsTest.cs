    ...
    public class HttpClientPostExtensionsTest
    {
        ...
        public async Task PostAsMessagePackAsync_String_WhenClientIsNull_ThrowsException()
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() =>
                ((HttpClient) null).PostAsMessagePackAsync(_uri, new object()));
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() =>
                _client.PostAsMessagePackAsync(_uri, new object(), null, CancellationToken.None));
        ...
        public async Task PostAsMessagePackAsync_String_WhenUriIsNull_ThrowsException()
        {
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
                _client.PostAsMessagePackAsync((string) null, new object()));
            Assert.Equal(
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() =>
                ((HttpClient) null).PostAsMessagePackAsync(new Uri(_uri), new object()));
            Assert.Equal("client", exception.ParamName);
        }
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() =>
                _client.PostAsMessagePackAsync(new Uri(_uri), new object(), null, CancellationToken.None));
            Assert.Equal("options", exception.ParamName);
        }
        ...
        public async Task PostAsMessagePackAsync_Uri_WhenUriIsNull_ThrowsException()
        {
            var exception =
                await Assert.ThrowsAsync<InvalidOperationException>(() =>
                    _client.PostAsMessagePackAsync((Uri) null, new object()));
            Assert.Equal(
                "An invalid request URI was provided. The request URI must either be an absolute URI or BaseAddress must be set.",
                exception.Message);
        }
